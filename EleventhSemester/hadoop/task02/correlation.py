...
def main():
    ...
    # Устанавливаем время начала и конца для вычислений
    ...
    # Фильтруем только нужные по времени
    ).filter(
    ...
    # Получаем список интервалов и сдвигов
    ...
    for width in args.widths:
        ...
        # Выделяем свечи для разных инструментов
        candle_map = {
        ...
        # Перебираем все комбинации инструментов и сдвигов
        for i, sec1 in enumerate(instruments):
            ...
            for sec2 in instruments[i + 1:]:
                for sshift in shifts:
                    df2 = (candle_map[sec2]
                           .withColumnRenamed('PRICE_DEAL', 'right_price')
                           .withColumnRenamed('candle_id', 'right_candle_id')
                          )

                    # Попарно джойним таблицы
                    joined = df1.join(df2, df1.left_candle_id == df2.right_candle_id + sshift, 'inner')

                    # Сохраняем результат джойна в список
                    values_list.append(joined.select(
                            'left_price',
                            'left_candle_id',
                            'right_price',
                            'right_candle_id',
                            f.lit(sec1).alias('sec1'),
                            f.lit(sec2).alias('sec2'),
                            f.lit(width).alias('width'),
                            f.lit(sshift).alias('sshift'),
                        ).rdd
                    )
    ...
    df_result = spark.createDataFrame(spark.sparkContext.union(values_list), StructType([
        StructField('left_price', DoubleType(), False),
        StructField('left_candle_id', IntegerType(), False),
        StructField('right_price', DoubleType(), False),
        StructField('right_candle_id', IntegerType(), False),
        StructField('sec1', StringType(), False),
        StructField('sec2', StringType(), False),
        StructField('width', IntegerType(), False),
        StructField('sshift', IntegerType(), False)
    ...
    # Аггрегируем с помощью корреляции
    ).groupBy(
