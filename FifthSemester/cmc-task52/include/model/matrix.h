...
public:
    ...
    bool
    in_matrix(const Coord &coord) const;
    ...
    const ValueT &operator() (const Coord &coord) const;
    // a(0, 1) = 3;
    // cout << a; // 9 3 7
    ValueT &operator() (ssize_t row, ssize_t col);
    ValueT &operator() (const Coord &coord);
...
private:
    ...
    template <typename T>
    inline T&
    make_rw(const T& val) const;
};

// Output for matrix. Useful for debugging
template <typename ValueT>
std::ostream
&operator<< (std::ostream &out, const Matrix<ValueT> &m);
