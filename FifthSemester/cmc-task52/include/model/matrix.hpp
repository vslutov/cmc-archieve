...
template <typename ValueT>
bool
Matrix<ValueT>::in_matrix(const Coord &coord) const
{
    return 0 <= coord.first && coord.first < this->n_rows &&
           0 <= coord.second && coord.second < this->n_cols;
}

template <typename ValueT>
ValueT &
Matrix<ValueT>::operator() (const Coord &coord)
{
    if (!this->in_matrix(coord)) {
        throw std::string("Out of bounds");
    }
    ssize_t row = coord.first,
            col = coord.second;
    row += pin_row;
    col += pin_col;
...
template <typename ValueT>
ValueT &
Matrix<ValueT>::operator() (ssize_t row, ssize_t col)
{
...
template <typename ValueT>
const ValueT &
Matrix<ValueT>::operator() (const Coord &coord) const
{
    if (!this->in_matrix(coord)) {
        throw std::string("Out of bounds");
    }
...
template <typename ValueT>
const ValueT &
Matrix<ValueT>::operator() (ssize_t row, ssize_t col) const
{
    return (*this)(Coord(row, col));
}
