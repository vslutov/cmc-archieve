...
template<typename ValueT>
Matrix<ValueT>::Matrix(ssize_t row_count, ssize_t col_count) :
    n_rows{ row_count },
    n_cols{ col_count },
    stride{ n_cols },
    pin_row{ 0 },
    ...
    _data{}
{
    ...
    // local block to invalidate stack variables after it ends.
    {
        ...
        auto chk_length = [local_n_cols](const std::initializer_list<ValueT> &l) {
            return static_cast<ssize_t>(l.size()) == local_n_cols;
        };
...
template<typename ValueT>
ValueT &Matrix<ValueT>::operator()(ssize_t row, ssize_t col)
{
...
template<typename ValueT>
const ValueT &Matrix<ValueT>::operator()(ssize_t row, ssize_t col) const
{
    if (row >= n_rows || col >= n_cols)
        throw std::string("Out of bounds");
    row += pin_row;
...
// Matrix<typename std::result_of<BinaryMatrixOperator(Matrix<ValueT>)>::type>
...
template<typename ValueT>
Matrix<ValueT> Matrix<ValueT>::extra_borders(ssize_t kernel_vert_radius, ssize_t kernel_hor_radius) const
{
    ...
    //top and bottom
    for (ssize_t i = 0; i < kernel_vert_radius; i++) {
        for (ssize_t j = 0; j < n_cols; j++) {
    ...
    //left and right
    for (ssize_t i = 0; i < n_rows; i++) {
        for (ssize_t j = 0; j < kernel_hor_radius; j++) {
...
{
    auto result = Matrix<ValueT>(this->n_cols, this->n_rows);
    for (ssize_t i = 0; i < this->n_rows; ++ i) {
        for (ssize_t j = 0; j < this->n_cols; ++ j) {
            result(j, i) = (*this)(i, j);
        }
    }
    return result;
